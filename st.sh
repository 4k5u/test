...
#userIds=$1
userIds="student-se Ryo-sama IdaJonesy Daji-Lovely RoseVal Baby_Puppy TuTu-1001 SexyModel-170cm grandeeney ii1liii1il asuna_love JessicaRewan Ailen_baby sabrina_hk888 lucy_1811 EmiliaGuess Stbeautn_oO charming_NaNa21 Judy0523 StunnerBeauty-170cm M--I--A alicelebel Reaowna Akiyama_Key sigmasian Xenomy TefFfish Sime_Naughty SanySenise _PunPun18 BadAngels666 JP-KARIN 777YikuYiku Hahaha_ha2 Daji-520 San___San mikio_san AkiShina Sherry_niko Lucille_evans morphesoull";
    ...
    for userId in ${userIds}; do
        ...
        if [ "${islive}" == true ]; then
            ...
            if [ -n "$roomid"  ] && [ "$roomid" != null ]; then
                ...
                movieid=$(curl -sSL --connect-timeout 5 --retry-delay 3 --retry 3 -H 'accept:application/json, text/plain, */*' -H "authorization:${roomToken}" -d "{\"id\": \"$movieid\"}" "${synctv}/api/movie/current")
