...
####################echo -e "添加$userId $hls">> $logfile
                    ...
                    curl -H 'Content-Type: application/json' -d "{\"chat_id\": \"1138220708\", \"caption\":\"$text\", \"photo\":\"https://cbjpeg.stream.highwebmedia.com/stream?room=${userId}\"}" "https://api.telegram.org/${bot}/sendPhoto?parse_mode=MarkdownV2"
